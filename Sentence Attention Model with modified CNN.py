...
directory = "/scratchd/home/swayambhu/Time_Stamping/Dumped_files/Sent_Attn_CNN_40k/"
container_directory = "/scratchd/home/swayambhu/Time_Stamping/Dumped_files/Sent_Attn_CNN_40k/List_of_Documents/"
test_directory = "/scratchd/home/swayambhu/Time_Stamping/Dumped_files/Sent_Attn_CNN_40k/Test_Documents/"
...
class CNNwithPool(nn.Module):
    def __init__(self, cnn_layers, kernel_size0, kernel_size1, kernel_size2):
        ...
        self.cnn0 = nn.Conv2d(1, cnn_layers, kernel_size0,stride = (1,1))
        self.cnn0.bias.data.copy_(weight_init.constant(self.cnn0.bias.data, 0.))
        self.cnn1 = nn.Conv2d(1, cnn_layers, kernel_size1,stride = (1,1))
        self.cnn1.bias.data.copy_(weight_init.constant(self.cnn1.bias.data, 0.))
    def forward(self, x):
        cn0 = self.cnn0(x)
        max_pool_size00 = cn0.size(2)
        max_pool_size01 = cn0.size(3)
        pooled_list0 = F.max_pool2d(cn0, (max_pool_size00, max_pool_size01))
        cn1 = self.cnn1(x)
        max_pool_size10 = cn1.size(2)
        max_pool_size11 = cn1.size(3)
        pooled_list1 = F.max_pool2d(cn1, (max_pool_size10, max_pool_size11))
...
def trainModel_mod_mod(container_directory, w2v_array, w2v_length, embedding_length, cnn_layer, kernel_size0, kernel_size1, kernel_size2, batch_size, num_of_class, hidden_layer_1, hidden_layer_2, epochs):
    ...
    for epoch in range(epochs):
        ...
        torch.save({'epoch': epoch,'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, directory+"Mod_CNN_Models/model_"+str(epoch))
...
# for ep in range(0):
   # epoch = 99
    checkpoint = torch.load(directory+"Mod_CNN_Models/model_"+str(epoch), map_location=lambda storage, loc: storage)
