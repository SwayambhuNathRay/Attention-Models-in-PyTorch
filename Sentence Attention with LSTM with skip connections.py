...
directory = "/scratchd/home/swayambhu/Time_Stamping/Dumped_files/Sent_Attn_CNN_LSTM_40k/"
container_directory = "/scratchd/home/swayambhu/Time_Stamping/Dumped_files/Sent_Attn_CNN_LSTM_40k/List_of_Documents/"
test_directory = "/scratchd/home/swayambhu/Time_Stamping/Dumped_files/Sent_Attn_CNN_LSTM_40k/Test_Documents/"
...
class SentenceAttention(nn.Module):
    ...
    def forward(self,cn, sent_vec, document_index, batch_size):
        out = []
        sentence_embedding_size = sent_vec.size(2)
        max_doc_len = sent_vec.size(1)
        new_cn = cn.transpose(0,1).view(batch_size, -1)
        new_sent_vec = sent_vec.view(batch_size, -1)
        x = torch.cat((new_cn, new_sent_vec),dim = 1).view(batch_size, max_doc_len, -1)
        for index, start_end in enumerate(document_index):
            bag = x[index][start_end[0] : start_end[1]]
            keep_sent_vec = sent_vec[index][start_end[0] : start_end[1]]
            ...
            sentence_vector = attention_values_softmax*keep_sent_vec
...
class final(nn.Module):
    def __init__(self, w2v_array, max_doc_len, w2v_length, embedding_length, cnn_layer, kernel_size0, kernel_size1, kernel_size2, num_of_class, hidden1, hidden2):
        ...
        self.attention = SentenceAttention(6*self.cnn_layer)
    ...
    def forward(self, x, document_index, batch_size):
        ...
        attention, given_attention = self.attention(cn, sent_vec, document_index, batch_size)
...
#                 print(p.grad)
        ...
        torch.save({'epoch': epoch,'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, directory+"Skip_Models/model_"+str(epoch))
    return model
...
for epoch in range(epochs):
    checkpoint = torch.load(directory+"Skip_Models/model_"+str(epoch), map_location=lambda storage, loc: storage)
